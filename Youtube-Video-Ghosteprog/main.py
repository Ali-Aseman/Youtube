...
# Window Setting
root = Tk()
root.iconbitmap("Images/anonymous.ico")
root.title("GHOSTEPROG")
...
image = ImageTk.PhotoImage(Image.open("images/icon.jpg"))
imageLabel = Label(root, image=image, bg="black")
imageLabel.grid(row=0, column=0, sticky=E)
heading = Label(root, text='Youtube Video Downloader | GHOSTEPROG', font=heading_font, bg="black", fg="white")
# heading_2 = Label(root, text='GHOSTEPROG', font=heading_font, bg="black", fg="white")
heading.grid(row=0, column=1, padx=0)
# heading_2.grid(row=1, column=1, padx=1)

url_label = Label(root, text='Video URL', font=('bold', 14), bg="black", fg="white")
url_label.grid(row=1, column=0, padx=0)
url_text = StringVar()
url_entry = Entry(root, textvariable=url_text, width=55, borderwidth=1, font=('bold', 14))
url_entry.grid(row=1, column=1)

search_button = Button(root, text='Search', bg='white', command=thread_search,
                           fg='black', font=('bold', 13), width=20)
